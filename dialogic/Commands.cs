...
namespace Dialogic
{
	...
	public abstract class Command : Meta
	{
		...
		internal static Command Create(Type type, string txt, string lbl, string[] metas)
		{
			...
			// ---------------- optimization ----------------
			if (type == typeof(Chat)) cmd = new Chat();
			else if (type == typeof(Say)) cmd = new Say();
			else if (type == typeof(Set)) cmd = new Set();
			else if (type == typeof(Ask)) cmd = new Ask();
			else if (type == typeof(Opt)) cmd = new Opt();
			else if (type == typeof(Do)) cmd = new Do();
			else if (type == typeof(Go)) cmd = new Go();
			else if (type == typeof(Wait)) cmd = new Wait();
			else if (type == typeof(Find)) cmd = new Find();
			// ----------------- end opt -------------------

			else cmd = (Command)Activator.CreateInstance(type);
