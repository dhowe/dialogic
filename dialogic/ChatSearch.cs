...
namespace Dialogic
{
    public static class ChatSearch
    {
         ...
         */
        public static Chat Find(List<Chat> chats, Dictionary<string, Constraint> constraints)
        {
         ...
         */
        public static List<Chat> FindAll(List<Chat> chats, Dictionary<string, Constraint> constraints)
        {
            ...
            for (int i = 0; i < chats.Count; i++)
            {
                ...
                foreach (var constraint in constraints.Keys)
                {
                    ...
                    {
                        var chatMetaVal = conditions[constraint];
                        // double-check required here for strings as objects
                        if (!constraints[constraint].Check(chatMetaVal))// != constraint.Value && !Equals(conditions[constraint.Key], constraint.Value))
                        {
