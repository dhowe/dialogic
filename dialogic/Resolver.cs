	...
	public class Resolver
	{
			...
			IDictionary<string, object> globals, int level = 0)
		{
			...
			while (symbols.Count > 0)
			{
				...
				var result = symbol.Resolve(globals, false); // TODO: combine
				if (result != null) text = symbol.Replace(text, result, globals);
            
				if (DBUG) Console.WriteLine("      " + symbol.Name() + " -> " + result + " :: " + text);

				// if we've made progress, and have no more symbols in our list, 
				// but there still have symbols in the text, then parse and repeat
